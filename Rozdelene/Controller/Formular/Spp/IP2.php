 ...
 * @author pes2704
 */
class Projektor2_Controller_Formular_Spp_IP2 extends Projektor2_Controller_Formular_IP {
    ...
    protected function formular() {
        ...
        $view->assign('nadpis', 'UKONČENÍ ÚČASTI V PROJEKTU A DOPLNĚNÍ IP - 2. část')
            ->assign('formAction', 'spp_ukonceni_uc')
     ...
     protected function getResultPdf() {
        ...
        if ($this->request->post('pdf') == "Tiskni IP 2.část - vyhodnocení aktivit") {
            ...
            $view = new Projektor2_View_PDF_Spp_IP2($this->sessionStatus, $this->createContextFromModels());
            $file = 'IP_cast2';
...
//                ->assign('aktivityProjektuTypuKurz', $aktivityProjektuTypuKurz)
        ...
        if ($this->request->post('pdf') == "Tiskni ukončení účasti") {
            $view = new Projektor2_View_PDF_Spp_Ukonceni($this->sessionStatus, $this->createContextFromModels());
            $file = 'ukonceni';
            //status proměnné
            $view->assign('kancelar_plny_text', $this->sessionStatus->getUserStatus()->getKancelar()->plny_text);
