 ...
 * @author pes2704
 */
class Projektor2_View_PDF_Vzp_Ukonceni extends Projektor2_View_PDF_Common {
    const MODEL_UKONCENI = "ukonceni->";
    const MODEL_DOTAZNIK = "dotaznik->";
    
    public function createPDFObject() {
        $textPaticky = "Ukončení účasti účastníka v programu ".$this->context["file"].$this->context["file"];   
        $this->createHeaderFooter($this->sessionStatus->getUserStatus()->getProjekt(), $textPaticky);
        $this->initialize();
        //*****************************************************
        $textyNadpisu[] = "UKONČENÍ ÚČASTI V PROGRAMU";
        $textyNadpisu[] = 'Program Vykroč za prací projektu „Vzdělávání a dovednosti pro trh práce II“';
        ...
        $blok = new Block;
            ...
            $blok->PridejOdstavec("Účastník ukončil účast v programu Vykroč za prací projektu „Vzdělávání a dovednosti pro trh práce II“ z dále uvedeného důvodu. "
                    . "Ke dni ukončení účasti skončila účinnost Dohody o účasti v programu Vykroč za prací projektu „Vzdělávání a dovednosti pro trh práce II“.");
        ...
        $ukonceniUcasti = new SadaBunek();
            $ukonceniUcasti->Nadpis("Údaje o účasti v programu");
    //    	$ukonceniUcasti->PridejBunku("Datum zahájení účasti v projektu: ", @$pdfpole["datum_reg"]);
            $ukonceniUcasti->PridejBunku("Datum zahájení účasti v programu: ", @$this->context[self::MODEL_DOTAZNIK  . "datum_vytvor_smlouvy"]);
            $ukonceniUcasti->PridejBunku(" Datum ukončení účasti v programu: ", @$this->context[self::MODEL_UKONCENI . 'datum_ukonceni'], 1);

            $duvod_ukonceni_pole =  explode ("|", @$this->context[self::MODEL_UKONCENI . 'duvod_ukonceni']);
            $ukonceniUcasti->PridejBunku("Důvod ukončení účasti v programu: ", $duvod_ukonceni_pole[0],1);
            if ( ($duvod_ukonceni_pole[0] == "2b ") or ($duvod_ukonceni_pole[0]== "3a ")  or ($duvod_ukonceni_pole[0] == "3b ")
                ...
                $ukonceniUcasti->PridejBunku("Podrobnější popis důvodu ukončení účasti v programu: ", " " ,1);
                $ukonceniUcasti1 = new Block;
                $ukonceniUcasti1->Odstavec( @$this->context[self::MODEL_UKONCENI . 'popis_ukonceni']);
            }
        $this->pdfRenderer->renderCellGroup($ukonceniUcasti);
        if ($ukonceniUcasti1) $this->pdfRenderer->renderBlock($ukonceniUcasti1);
        $pozn = new Block;
            $pozn->Nadpis("Možné důvody:");
            $pozn->Odstavec("1. řádné absolvování programu");
        ...
        $pozn = new Block;
            ...
            $pozn->PridejOdstavec("b. výpovědí dohody o účasti v programu účastníkem nebo ukončením dohody z jiného důvodu než nástupu do zaměstnání (ukončení bude v den předcházející dni vzniku důvodu ukončení)");
            $pozn->VyskaPismaTextu(8);
        ...
        $this->pdfRenderer->renderBlock($pozn);
        $pozn = new Block;
            $pozn->Odstavec("a. pokud účastník porušuje podmínky účasti v programu, neplní své povinnosti při účasti na aktivitách projektu (zejména na rekvalifikaci) nebo jiným závažným způsobem maří účel účasti v programu");
