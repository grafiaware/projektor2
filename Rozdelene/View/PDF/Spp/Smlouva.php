 ...
 * @author pes2704
 */
class Projektor2_View_PDF_Spp_Smlouva extends Projektor2_View_PDF_Common {
    const MODEL_SMLOUVA = "smlouva->";   //--vs
    
    public function createPDFObject() {
        $textPaticky = "Dohoda o účasti v projektu „Šance pro padesátníky v Plzeňském kraji“ ".$this->context["file"];
        $this->createHeaderFooter($this->sessionStatus->getUserStatus()->getProjekt(), $textPaticky);
        $this->initialize();
        //*****************************************************
        $textyNadpisu[] = "DOHODA O ÚČASTI V PROJEKTU ";
        $textyNadpisu[] = 'Projekt „Šance pro padesátníky v Plzeňském kraji“';
        ...
        $blok = clone $defaultBlokCislovani2;
            ...
            $blok->Odstavec("1.1 Účelem dohody uzavírané ve smyslu ustanovení § 51 zákona č. 40/1964 Sb., občanského zákoníku, ve znění pozdějších předpisů, a také na základě zákona č. 435/2004 Sb., o zaměstnanosti, a podle souvisejících předpisů, je úprava vzájemného vztahu mezi účastníkem a Příjemcem při provádění aktivit projektu pro fyzické osoby účastnící se grantového projektu implementovaném v Operačním programu Lidské zdroje a zaměstnanost: „Šance pro padesátníky v Plzeňském kraji“.");
// ??
            $blok->PridejOdstavec("1.2 Projekt představuje soubor aktivit a nástrojů určených pro dosažení cílů projektu. Hlavním cílem „Šance pro padesátníky v Plzeňském kraji“ je snižování nezaměstnanosti a předcházení dlouhodobé nezaměstnanosti v Plzeňském kraji. Projekt je zaměřen na udržení a obnovení profesní orientace nebo získání nových dovedností a posílení profesního sebevědomí klientů, kteří díky svému věku a dalším handicapům potřebují pomoc při udržení na trhu práce.");
        $this->pdfRenderer->renderBlock($blok);

        $blok = clone $defaultBlokCislovani2;
            $blok->Nadpis("2. Předmět dohody");
            $blok->Odstavec("2.1 Předmětem dohody je stanovení podmínek účasti Účastníka na aktivitách projektu „Šance pro padesátníky v Plzeňském kraji“ a úprava práv a povinností Stran dohody při realizaci těchto aktivit.");
        $this->pdfRenderer->renderBlock($blok);

        $blok = clone $defaultBlokCislovani2;
            $blok->Nadpis("3. Povinnosti a práva účastníka projektu „Šance pro padesátníky v Plzeňském kraji“");
            $blok->Odstavec("3.1 Účastník potvrzuje, že se účastnil výběrové schůzky, kde byl seznámen s prezentací projektu „Šance pro padesátníky v Plzeňském kraji“.");
        ...
        $blok = clone $odsazenyBlokCislovani2;
            ...
            $blok->PridejOdstavec("c) účastnit se kurzů projektu „Šance pro padesátníky v Plzeňském kraji“.");
        $this->pdfRenderer->renderBlock($blok);
        ...
        $blok = clone $defaultBlokCislovani2;
            ...
            $blok->Odstavec("4.1 K ukončení účasti účastníka v projektu „Šance pro padesátníky v Plzeňském kraji“ dojde v následujících případech:");
        $this->pdfRenderer->renderBlock($blok);
        ...
        $blok = clone $defaultBlokCislovani2;
            ...
            $blok->PridejOdstavec("4.3 Účastník se zavazuje, že se dostaví do Konzultačního centra a podepíše doklad o ukončení účasti účastníka v projektu „Šance pro padesátníky v Plzeňském kraji“ pokud nebude dohodnuto jinak. Přílohou tohoto dokladu bude například kopie pracovní smlouvy, kopie výpovědi, atd.");
            $blok->PridejOdstavec("4.4 Po ukončení účasti účastníka v projektu „Šance pro padesátníky v Plzeňském kraji“ řádným způsobem anebo z důvodu nástupu do zaměstnání získá účastník Osvědčení o absolvování projektu „Šance pro padesátníky v Plzeňském kraji“.");
// ??
...
// ??            
            $blok->Odstavec("5.1. Účastníkovi mohou být při účasti na aktivitách projektu poskytovány příspěvky na náhradu některých nákladů souvisejících s účastí v projektu (tzv. přímá podpora), a to za podmínek stanovených projektem „Šance pro padesátníky v Plzeňském kraji“. Jedná se o tyto příspěvky:");
        $this->pdfRenderer->renderBlock($blok);
...
// ??        
//        $blok = clone $defaultBlokCislovani2;
//            $blok->Odstavec("5.2. Bližší specifikace uvedených druhů přímé podpory je uvedena v dokumentu Základní informace účastníka projektu „Šance pro padesátníky v Plzeňském kraji“.");
